        ...
        <div id = "pp-afterslider" class = "entry-content">
            <section id = "container" class="two-columns-right">
                ...
                <div id = "content" role="main">
                    ...
                    if ($nirvana_fronttext1 || $nirvana_fronttext3) { ?>
                        <div id = "pp-texttop">
<!--                        <div class = "cfit-homepage-left">-->
                            <?php
                            if ($nirvana_fronttext1) { ?>
                                <div id = "front-text1" class = "ppbox">
                                <h2><?php echo do_shortcode($nirvana_fronttext1) ?> </h2>
                                </div><?php }
                            if ($nirvana_fronttext3) { ?>
                                <div id = "front-text3" class = "ppbox"> <?php echo do_shortcode($nirvana_fronttext3) ?></div><?php }
                            ?>
<!--                        </div>-->
<!--                        <div class = "cfit-homepage-right">-->
...
<!--                        </div>-->
                    ...
                    if ($nirvana_fronttext2 || $nirvana_fronttext4) { ?>
                        <div id = "pp-textmiddle"><?php
                        // Second FrontPage title
                        if ($nirvana_fronttext2) { ?>
                            <div id = "front-text2" class = "ppbox">
                            <h2><?php echo do_shortcode($nirvana_fronttext2) ?> </h2>
                            </div><?php }
                        // Frontpage second text area
                        if ($nirvana_fronttext4) { ?>
                            <div id = "front-text4" class = "ppbox"> <?php echo do_shortcode($nirvana_fronttext4) ?> </div><?php }
                        ?></div><?php }
                    ...
                    if ($nirvana_fronttext5 || $nirvana_fronttext6) { ?>
                        <div id = "pp-textbottom"><?php
                        if ($nirvana_fronttext5) { ?>
                            <div id = "front-text5" class = "ppbox">
                            <h2><?php echo do_shortcode($nirvana_fronttext5) ?> </h2>
                            </div><?php }
                        if ($nirvana_fronttext6) { ?>
                            <div id = "front-text6" class = "ppbox"> <?php echo do_shortcode($nirvana_fronttext6) ?></div><?php }
                        ?></div><?php } ?>
                </div>
        </div> <!-- #content -->
            ...
            <div id = "slider" class = "nivoSlider">
                <?php foreach ($slides as $id => $slide):
                    ...
                    <a href = '<?php echo($slide['link'] ? $slide['link'] : '#'); ?>'>
                        <img src = '<?php echo $slide['image']; ?>' data-thumb = '<?php echo $slide['image']; ?>'
                                alt = "<?php echo($slide['title'] ? esc_attr($slide['title']) : ''); ?>"
                            <?php if ($slide['title'] || $slide['text']): ?>title = "#caption<?php echo $id; ?>" <?php endif; ?> />
                        </a><?php endif; ?>
                <?php endforeach; ?>
            </div>
            <?php foreach ($slides as $id => $slide): ?>
                <div id = "caption<?php echo $id; ?>" class = "nivo-html-caption">
                    ...
                    <?php if ($nirvana_slidereadmore && $slide['link']): ?>
                        <div class = "readmore">
                            <a href = "<?php echo esc_url($slide['link']) ?>"><?php echo esc_attr($nirvana_slidereadmore) ?>
                                <i class = "column-arrow"></i></a>
                        </div>
                    <?php endif; ?>
                </div>
