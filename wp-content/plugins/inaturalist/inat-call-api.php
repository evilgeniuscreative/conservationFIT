...
// OBSERVATIONS LIST -- NOW IN inat-observations-list.php
...
{
    echo "<p>theme_list_single_obs</p>";
    $output = '  
...
/* Google maps -- NOW IN inat-map.php */
function theme_map_obs($data, $context = 'page')
{
    ...
    $output =
        '<div id="map"></div>
        <script>
        function initMap()';// ignore this warning
    $output .= '{';
    $output .= $startCenter;
    $output .= $startMap;
...
<div class="inat_observation_single" id="obs_' . $observation->id . '">
    ...
    if (observation_photos_count == 0) {
        ...
        $output .= '<div class="cycle-slideshow img-wrapper img-wrapper-' . $id . '"data-cycle-slides="> figure"data-cycle-fx=fade>';
...
{
    $output = '
<form accept-charset="UTF-8" id="inat-obs-add" method="post" action="' . plugins_url('add-obs.php', __FILE__) . '">
  <div>
