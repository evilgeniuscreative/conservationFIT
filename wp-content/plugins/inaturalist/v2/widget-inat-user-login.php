...
class INaturalist_Login extends WP_Widget
{
    ...
    function __construct()
    {
        $widget_ops = array(
            ...
            'description' => 'Log in to iNaturalist in order to upload observations, etc.'
        );
        parent::__construct(
            'inaturalist_login', 'iNaturalist Login', $widget_ops
        );
    }
    ...
    public function widget($args, $instance)
    {
        ...
        if ($current_user->user_login) {
            ...
            if (isset($_COOKIE['inat_access_token'])) {
                // is logged in to iNat
                echo '<p class="inat-logged-in">You are logged into iNaturalist as <a href="https://inaturalist.org/users/'.$current_user->user_login.'" title="Your profile at iNaturalist.org" target="_blank">'.$current_user->user_login.'</a></p>';
            } else {
                // is not logged in to iNat, so give them the option
...
function inat_user_login()
{
    ...
    if (isset($_POST['inat_username']) && isset($_POST['inat_password'])) {
        ...
        //setcookie('inat_access_token',$token,time()+3600);
//if(!headers_sent()){
//    echo "no headers";
//} else {
//    echo "headers sent";
//}
        ...
        wp_redirect($_POST['redirect'].'?inat_key='.$token);
    }
}
